...
class Polygon: public Primitive {
    ...
    __host__ __device__ Polygon(int nVertices, Vector *vert, Properties p);
    __host__ __device__ Polygon(const Polygon &pln);
    // __host__ __device__ ~Polygon() { delete vertices; };
    __host__ __device__ ~Polygon()=default;
};


/*----------------------------------FUNCTIONS----------------------------------*/

__device__ void Polygon::rayIntersection(Ray *ray, Primitive **object) {
    //Computes polygon plane
    Vector u = vertices[1] - vertices[0];
    Plane p1(vertices[0], N); 
...
__device__ Vector Polygon::getNormal(Vector P) { return N; }

__host__ __device__ void Polygon::print() { 
    printf("Polygon [");
...
/*--------------------------------CONSTRUCTORS---------------------------------*/
__host__ __device__ Polygon::Polygon() {};
__host__ __device__ Polygon::Polygon(int nVertices, Vector *vert, Properties p) : 
...
__host__ __device__ Polygon::Polygon(const Polygon &cln) { *this = cln; }
